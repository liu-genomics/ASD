...
rr is estimated from top 6% TADA genes.
```{r}
...
## Number of gained and lost genes, remove genes that don't have have splicing mutations or noncoding mutations, and don't have K27ac
...
## *cacluate the bayes factor table for splicing mutations, use RR 2.4 for splicing mutaitons
...
## *cacluate the bayes factor table for splicing mutations, summing mutation rate from ChrX and ChrY for a gene if it is on both ChrX and ChrY, splicing RR 2.4* 
...
## *now combine information from splicing (collapsed version) to other non_coding mutations, splicing RR 2.4*
```{r}
combined_BF_table <- splicing_BF_table_collapsed[TADA_table$gene_BF_table, on = "genename"]
combined_BF_table
...
## *Sart without using splicing mutations, rr from top 1000 genes*
rr is estimated from top 1000 TADA genes.
```{r}
TADA_table<-bayes_factor_for_each_gene_base_level_from_collapsed_data(adjusted_base_level_rr$base_info, 
                                                                      features = c(1), 
                                                                      rr = 0.7209638,
...
## *cacluate the bayes factor table for splicing mutations, use RR exp(1.5368668) for splicing mutaitons
RR estimated in a model-based way see [here](170329_base_level_mutation_info_with_3_epi_and_2_conservation_splicing_top1000genes_test_genename_assignment_corrected.html)
