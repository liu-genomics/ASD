...
5. This analysis is done to compare the consistency between [estimate_effect_size_for_simulation_data_mixture_with_categorization_for_big_data] and [estimate_effect_size_for_simulation_data_mixture_with_categorization_v2] on small dataset. 


***

##### First try a toy model generating a dataset with 1000base/gene, 500 genes, effect size 2
##### 
```{r, cache=TRUE, cache.extra = estimate_effect_size_for_simulation_data_mixture_with_categorization_for_big_data, cache.extra = estimate_effect_size_for_simulation_data_mixture_with_categorization_v2, cache.extra = mutation_simulator}

# mutation_simulator to only have 1 epigegnomic feature added, the prior is 0.3 for each gene
mutation_count <- mutation_simulator(gene_number = 500,window_number=1000,feature_prob = 0.2,base_rate = 2e-6,sample_size = 314, p_risk = 0.3, effect_size = 2)
...
fwrite(data, paste(prefix, "_temp_mut_info.txt", sep = ""), col.names = TRUE, row.names = FALSE, sep = "\t", quote = FALSE)
...
# categorization optimization without big data approach

data <- as.data.table(data)

simulation_estimate <- estimate_effect_size_for_simulation_data_mixture_with_categorization_v2(data = data, feature_start = 6, feature_end = 6, feature_number = 1, gene_prior_file = paste(prefix, "_temp_prior_for_simulation.txt", sep = ""), sample_size = sample_size)

# categorization optimization with big data approach
simulation_estimate_big_data <- estimate_effect_size_for_simulation_data_mixture_with_categorization_for_big_data(data_file = paste(prefix, "_temp_mut_info.txt", sep = ""), feature_start = 6, feature_end = 6, feature_number = 1, gene_prior_file = paste(prefix, "_temp_prior_for_simulation.txt", sep = ""), sample_size = sample_size)
rm(data)
rm(mutation_count)
system(paste("rm ", paste(prefix, "_temp_prior_for_simulation.txt", sep = ""), sep = ""))
system(paste("rm ", paste(prefix, "_temp_mut_info.txt", sep = ""), sep = ""))
```

```{r, dependson = simulation_estimate,simulation_estimate_big_data }
simulation_estimate
simulation_estimate_big_data
```
...
```{r,dependson=simulation_estimate_big_data}
simulation_estimate
...
##### estimate relative risk from top 6% TADA genes, afeter adjusting for mutation rates, using the big_data version of optimization function, and RCC version of base-level-infor generating function

```{r, cache=TRUE, cache.extra = adjusted_mutation_rate_window_to_base_RCC, cache.extra = estimate_effect_size_for_simulation_data_mixture_with_categorization_for_big_data}
new_control = new.env()
load("../data/debug_region_list_073116_using_258_control_data_matrix.Rdata", new_control)
...
preprocessing_time <- system.time(adjusted_base_level_mutrate <- adjust_mutation_rate_window_to_base_RCC(mut_file = paste(prefix,"temp.bed",sep = ""),
window_file = "../other_annotation/epigenomic_annotation/Whole_genome.promoter_yanyu_10kb_exons_no_utr_no_na_genes.50bp_window.bed",
cg_file = "../other_annotation/epigenomic_annotation/Whole_genome.promoter_yanyu_10kb_exons_no_utr_no_na_genes.50bp_window.bed.fasta.cg",
mutrate_file = "../other_annotation/epigenomic_annotation/Whole_genome.promoter_yanyu_10kb_exons_no_utr_no_na_genes.50bp_window.bed.mutrate",
sample_size = 314,
...
mutrate_ref_file = "/media/yuwen/Elements/mutation_rate_raw/MarkDaly/Daly_mutrate.bw",
node_n = 6,
output_file = "130131_top6_pct_TADA_genes_base_level_info.txt"))


# transfomr mutation table

simulation_estimate_big_data <- estimate_effect_size_for_simulation_data_mixture_with_categorization_for_big_data(data_file = "130131_top6_pct_TADA_genes_base_level_info.txt", feature_start = 6, feature_end = 6, feature_number = 1, gene_prior_file = "../other_annotation/gene_list/TADA_SNV_CNV_combined_Feb7_tadaname_null_PPA_p0_0.94.txt", sample_size = 314)

rm(new_control)
system(paste("rm ",prefix,"annovar_input_temp.bed",sep = ""))
system(paste("rm ",prefix, "temp.bed",sep = ""))
system("rm 130131_top6_pct_TADA_genes_base_level_info.txt")
```

```{r,dependson=simulation_estimate_big_data}
simulation_estimate_big_data
fisher_info<-solve(-simulation_estimate_big_data$mle$hessian)
prop_sigma<-sqrt(diag(fisher_info))
upper<-simulation_estimate_big_data$mle$par+1.96*prop_sigma
lower<-simulation_estimate_big_data$mle$par-1.96*prop_sigma
a = data.frame(relative_risk = simulation_estimate_big_data$mle$par, lower_bound = lower, upper_bournd = upper)
rownames(a) = c("Noonan+Roadmap Brain H3K27ac enhancer + promoter")
knitr::kable(a)
```
