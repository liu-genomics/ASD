---
title: "Generte new TADA results"
author: "Yuwen Liu"
...
**Also the current function bayes_factor_for_each_gene doesn't take into account of splicing mutations that are not in the coding regions nor in the non-coding regions within 10kb of TSS.**
HGF intron mutations are missed as a result of this window-based analysis. 

This is because my windows don't have those windows that are in the introns that are far away from TSS. 
...
```{r, cache=TRUE, dependson=bayes_factor_for_each_gene}
...
TADA_table <- bayes_factor_for_each_gene(mut_file = paste(prefix,"temp.bed",sep = ""), 
...
```{r}
knitr::kable(TADA_table[TADA_table$FDR_all <= 0.1 & TADA_table$FDR_coding >0.1, -c(2,3,4)])
```

```{r}
nrow(TADA_table[TADA_table$FDR_all > 0.1 & TADA_table$FDR_coding <=0.1,])
```

# Change to previous estimates used in the paper. 
...
TADA_table2 <- bayes_factor_for_each_gene(mut_file = paste(prefix,"temp.bed",sep = ""), 
                                                                            window_file = "../other_annotation/epigenomic_annotation/Whole_genome.promoter_yanyu_10kb_exons_no_utr_no_na_genes.50bp_window.bed",
                                                                            cg_file = "../other_annotation/epigenomic_annotation/Whole_genome.promoter_yanyu_10kb_exons_no_utr_no_na_genes.50bp_window.bed.fasta.cg",
                                                                            mutrate_file = "../other_annotation/epigenomic_annotation/Whole_genome.promoter_yanyu_10kb_exons_no_utr_no_na_genes.50bp_window.bed.mutrate",
                                                                            sample_size = 314,
...
```{r}
knitr::kable(TADA_table2[TADA_table2$FDR_all <= 0.1 & TADA_table2$FDR_coding >0.1,-c(2,3,4)])
```

```{r}
nrow(TADA_table2[TADA_table2$FDR_all > 0.1 & TADA_table2$FDR_coding <=0.1,])
```
