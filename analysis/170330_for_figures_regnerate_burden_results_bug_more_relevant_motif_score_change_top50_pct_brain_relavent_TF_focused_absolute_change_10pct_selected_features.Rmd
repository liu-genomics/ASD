...
Make figures for Xin's presentation in the week of 170403, changes have been made based on his suggestions. 
...
temp = enhancer[c("ASD genes","Neuropsychiatric genes","nonASD genes"),]
...
temp$category = c(rownames(temp)[1:3])
temp$category = factor(temp$category, levels = temp$category)
temp = data.frame(temp, group = rep("Within 10kb",length(temp[,1])))
...
temp2$category = c(rownames(temp2)[1:3])
temp2$category = factor(temp2$category, levels = temp2$category)
temp2 = data.frame(temp2, group = rep("10kb~50kb",length(temp[,1])))
temp2 = temp2[c("ASD genes","Neuropsychiatric genes","nonASD genes"),]
...
p
ggsave("./tmp/170330_burden_by_geneset.pdf",width = 4, height = 4)
ggsave("./tmp/170330_burden_by_geneset.eps",width = 4, height = 4)
...
```{r,fig.width=5,fig.height=5,echo=FALSE}
temp = rbind(splicing_burden, splicing_control_burden)
temp$group = factor(temp$group, levels = c("Splicing mutations", "Control mutations"))
# notice there is categary labled with two spaces and with 1 space
temp = temp[c(2,3,4,5,6,11,12,16,14,15),]

temp$category = factor(temp$category, levels = c("Neuron Psychiatric genes", "nonASD genes", " ", "Intolerant genes", "Tolerant genes"))
...
ggsave("./tmp/170330_splicing_burden_lowest_priority.pdf",width = 4, height = 4)
ggsave("./tmp/170330_splicing_burden_lowest_priority.eps",width = 4, height = 4)
```
