...
sim_table_risk_nonrisk <- function(sample_size_per_gene = 100, gene_prior_file = "../other_annotation/gene_list/TADA_SNV_CNV_combined_Feb7_tadaname_null_PPA_p0_0.94.txt", relative_risk = c(0.5,0.8,0.9,1.1,1.3,0.1,0.2),
                                   optimization_prop = 0.1){
  ...
  active_effect <- as.matrix(predictors_temp_risk[,1:7])%*%relative_risk
  ...
  fr<-function(x){ # the function that will be optimized later for promoter_beta, enhancer_beta
    ...
    logP_Zg1 = by(predictors, predictors[,"genename"],  
                  function(x) sum(x$sim_mut_count*(log(x$adjusted_mutrate)+(as.matrix(x[,1:7])%*%all_rr))-x$adjusted_mutrate*exp((as.matrix(x[,1:7])%*%all_rr))-log(factorial(x$sim_mut_count))))
  ...
  run_time <- system.time(mle <- optim(rep(0.1,7), fr, control=list("fnscale"=-1), hessian = TRUE))
  
  list(mle = mle, run_time = run_time)
}

# The v2 version used less number of features, 3 binary and 1 continuous
sim_table_risk_nonrisk_v2 <- function(sample_size_per_gene = 100, gene_prior_file = "../other_annotation/gene_list/TADA_SNV_CNV_combined_Feb7_tadaname_null_PPA_p0_0.94.txt", relative_risk = c(1,1.5,2,0.3),
                                   optimization_prop = 0.1){
  #[sample_size_per_gene] is the number of bases under consideraton for each gene, 15000 for each gene if the summation over all genes expected to be about 3e8
  #[gene_prior_file] = "../other_annotation/gene_list/TADA_SNV_CNV_combined_Feb7_tadaname_null_PPA_p0_0.94.txt
  #[relative_risk] the increase of poisson rate if the gene is a risk gene
  ...
  list(mle = mle, par_interval = data.frame(lower_bound = lower, upper_bound = upper), run_time = run_time)
}
