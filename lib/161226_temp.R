...
verified_effect_size_estimate_noncoding_mutations_v2 <- function(mut_file, window_file, cg_file, mutrate_file, sample_size, epigenomic_marks = "no", overlap = 0.5, gene_assign_file,rm_nonsyn = FALSE, annovar_folder = "/media/yuwen/Elements/ANNOVAR/annovar/", annovar_input = "no.txt", gene_prior_file, annotation_file = "no"){
  ...
  # [epitenomic_marks] if it is "no", then glm won't use if a mutation is in h3k27ac as a categorical variable, if it is a vector of epigenomic annotation names, epigenomic mark will be added one by one.
  ...
  # [annotation_file] if it is "no", then no window-level annotation data such as phastcons score will be used. Other wise a vector of annotation scores will be used. 
  ...
  if(epigenomic_marks == "no"){
    ...
    fr<-function(x){ # the function that will be optimized later for promoter_beta, enhancer_beta
      ...
      all_rr = x
      logP_Zg1 = by(coverage, mutrate_temp[,"genename"],  
