...
# The RCC version of this function will partition big bed files to smaller ones, so that bigwigaverageoverbed and bedtools could handle. 
adjust_mutation_rate_window_to_base_RCC <- function(mut_file, window_file, cg_file, mutrate_file, sample_size, epigenomic_marks = "no", overlap = 0.5, rm_nonsyn = FALSE, annovar_folder = "/media/yuwen/Elements/ANNOVAR/annovar/", annovar_input = "no.txt", 
  ...
  # rm object that will not be used in the following steps to save memory usage
  rm(coverage)
  ...
  # remove files that won't be used in the next step to reduce memory usage.
  rm(coverage_noncoding)
  ...
  # write out a bed file to get base-level mutation rates, need to partition into smaller parts and then combine. 
