...
mutation_count = mutation_simulator(gene_number = 200,window_number=100,feature_prob = 0.2,base_rate = 2e-6,sample_size = 2000, p_risk = 1, effect_size = 100)

estimate_effect_size_for_simulation_data<-function(data){
  #[data] is the returned value from function{mutation_simulator}
  fr<-function(x){
    effect = x
    logP = by(data$data,data$data$gene, function(x) sum(x$mut_count*(log(x$base_mut_rate)+x$feature_indicator*effect+log(2*data$sample_size))-x$base_mut_rate*exp(x$feature_indicator*effect)*2*data$sample_size-log(factorial(x$mut_count))))
    -sum(logP)
  }
  mle = optim(1,fr,method = "Brent",lower = 0, upper = 10)
  mle
}
