...
# This function adjust mutation rate for 50-bp windows, including TSS-distant introns that have a spidex score.
# including nonsynonymous mutations is hard coded.
# including spdx TSS-distal introns is hard-coded.

adjust_mutation_rate_window <- function(mut_file, window_file, cg_file, mutrate_file, sample_size){
...
# This version allows for dealing with really big data, e.g., > 2 billion bases
# will read in [data_file] by chunk. 
# this version has more flexibility to choose the features that will be used to do foward model selection, or even for parameter estimation for a specific set of parameters. 
estimate_effect_size_for_simulation_data_mixture_with_categorization_for_compact_data_model_selection<-function(data, features = c(1,2), gene_prior_file, sample_size = 314){
  #[data] is the object returned from [adjust_mutation_rate_window_to_base_compact], which has compact base-level information. 
  #[features] the index of features that will be used in model training. For example, in the previous data preprocessing, there are five features, and I wanted to use the first and second, this argument would be c(1,2).
  # if [features] is set to be 0. Then the mutation generating model for risk genes would be same as for non-risk genes
  #[gene_prior_file], a file that has prior (derived from posterior and prior)for a gene as a risk gene. genename \t probability e.g. ../other_annotation/gene_list/TADA_SNV_CNV_combined_Feb7_tadaname_null_PPA_p0_0.94.txt
  ...
  if(features != 0){
    feature_number <- length(features)
    ...
    fr<-function(x){
      ...
      cal_logP_Zg1 <- function(data_partition_element){
        cal_logP_Zg1_level2 <-function(data_partition_element_level2){
          data_partition_element_level2[[2]]+data_partition_element_level2[[4]]*data_partition_element_level2[[1]][features]%*%all_rr-data_partition_element_level2[[3]]*exp(data_partition_element_level2[[1]][features]%*%all_rr)-data_partition_element_level2[[5]]
        }
        sum(sapply(data_partition_element, cal_logP_Zg1_level2))
      }
    ...
    return(list(optimization_time = optimization_time, mle = mle))  
  }
  else{ # if features == 0, i.e., doesn't have any parameters for relative risks of any features of risk genes. 
    cal_logP_Zg0 <- function(data_partition_element){
      cal_logP_Zg0_level2 <-function(data_partition_element_level2){
        data_partition_element_level2[[2]]-data_partition_element_level2[[3]]-data_partition_element_level2[[5]]
      }
    ...
    logP_table<-data.table(logP_Zg0 = logP_Zg0, genename = names(logP_Zg0))
    
    logP_table <- logP_table[gene_prior, on = "genename"]
    logP_table <- logP_table[complete.cases(logP_table)]
    
    ll <- sum(by(logP_table, logP_table$genename, function(x){sum(x$logP_Zg0)}))
...
# more features that might have effect size is added and binaried. 
# include even more features for epigenomic annotations
# will add splicing mutations info in this function
adjust_mutation_rate_window_to_base_compact_more_feature_splicing_hybrid <- function(mut_file, window_file, cg_file, mutrate_file, sample_size, epigenomic_marks = c("../other_annotation/epigenomic_annotation/Noonan_brain_roadmap_union.bed","../other_annotation/epigenomic_annotation/Epigenome_E081_E082_intersection.bed","../other_annotation/epigenomic_annotation/encode_DHS_union.bed"),
                                                                                     sequence_annotation = c("phylop_100way", "gerp"), 
                                                                                     sequence_annotation_cutoff = list(phylop_100way=2, gerp =2 ), sequence_annotation_ref = list(phylop_100way = "../other_annotation/conservation_annotation/hg19.100way.phyloP100way.bw", gerp = "../other_annotation/conservation_annotation/hg19_gerp_score.bw"),
                                                                                     overlap = 0.5, rm_nonsyn = FALSE, annovar_folder = "/media/yuwen/Elements/ANNOVAR/annovar/", annovar_input = "no.txt", 
                                                                                     gene_assign_file, gene_prior_file, report_proportion,  mutrate_ref_file = "../other_annotation/mutation_rate/Daly_mutrate.bw", 
                                                                                     node_n =6, feature_start = 5, feature_end = 6, feature_number = 2, chunk_partition_num =20, spidex_mutrate_prefix, splicing_cutoff, proband_SNV_mut_spidex){
  # [mut_file] is the mutation location file from one dataset. e.g.,../analysis/160229_data_for_analysis.bed, chr \t start \t end \t index,  1-based start and end
  # [window_file] is the file with genome partitioned into windows. e.g., ../other_annotation/epigenomic_annotation/Whole_genome.promoter_yanyu_10kb_exons.50bp_window.bed
  # [cg_file] is the file that has CG-content calculated, matching [window_file]. e.g., ../other_annotation/epigenomic_annotation/Whole_genome.promoter_yanyu_10kb_exons.50bp_window.bed.cg
  # [mutrate_file] is the file that has mutrate data. e.g., "../other_annotation/epigenomic_annotation/Whole_genome.promoter_yanyu_10kb_exons.50bp_window.bed.mutrate"
  ...
  # now get the part of coverage that doesn't include "spdex_intron"
  coverage_1 <- coverage[spdx_intron == 0, ]
  #coverage_2 <- coverage[spdx_intron == 1, ]
  
  # assign gene name to windows, it is only relevant for windows that don't have spdx_intron. 
  gene_assign <- fread(gene_assign_file, header = FALSE, sep = "\t", stringsAsFactors = FALSE)
  colnames(gene_assign) <- c("site_index", "genename")
  coverage_1 <- coverage_1[gene_assign, on = "site_index"]
  ...
  gene_prior$prior <- 1- gene_prior$prior
  # merge gene prior info
  coverage_1 <- coverage_1[gene_prior, on = "genename"]
  coverage_1 <-coverage_1[complete.cases(coverage_1)]
  ...
  if(report_proportion !=1){
    genes_for_report <- gene_prior[order(gene_prior[,2],decreasing = TRUE),1]
    genes_for_report <- genes_for_report[1:floor(length(genes_for_report)*report_proportion)]
    coverage_1 <- coverage_1[is.element(coverage_1$genename, genes_for_report),]
  }else{
    genes_for_report  <- gene_prior[,1] # choose all the genes in TADA coding table 
  }
  
  # get the adjustion effect for each window by dividing adjusted_mutrate by mutrate
  coverage_1$adjusting_effect = coverage_1$adjusted_mutrate/coverage_1$mutrate
  
  # now need to extropolate window mutation file to base level mutation file, now removes coding region, and don't differntiate between promoter and nf
  coverage_noncoding <-coverage_1[coding != 1,c("chr","start","end","adjusting_effect","genename"),with = FALSE]
  coverage_noncoding$ID <- paste(coverage_noncoding$genename, coverage_noncoding$start, sep = "_")
  ...
  for(i in 1:length(coverage_by_chr)){
    ...
    if(nrow(proband_SNV_mut_spidex_with_location_chr) == 0) next #if no mutations on this current chromosome are splicing mutations, exit the current iteration without adding anything to [splicing_data_partition] and go to the next one
  ...
  system(paste("rm ", prefix, "_temp*", sep = ""))
  list(partition_time1 = partition_time1, partition_time2 = partition_time2, base_info = data_partition, splicing_info = splicing_data_partition)
}
...
# This version allows for dealing with really big data, e.g., > 2 billion bases
# will read in [data_file] by chunk. 
# will take output from [adjust_mutation_rate_window_to_base_compact_more_feature_splicing_hybrid], which contains splicing info in a partitioned compact form
estimate_effect_size_for_simulation_data_mixture_with_categorization_for_compact_data_splicing_hybrid<-function(data, splicing_data, non_splicing_features , gene_prior_file, sample_size = 314){
  #[data] is the object [base_info] returned from [adjust_mutation_rate_window_to_base_compact_more_feature_splicing_hybrid], which has compact base-level information. 
  #[splicing_data] is the object [splicing_info] returned from [adjust_mutation_rate_window_to_base_compact_more_feature_splicing_hybrid]
  # [non_splicing_features] is a vector indicating which non-splicing features need to be estimated for RR. e.g., c(2,3), meaning the 2nd and 3rd feature.
  #[gene_prior_file], a file that has prior (derived from posterior and prior)for a gene as a risk gene. genename \t probability e.g. ../other_annotation/gene_list/TADA_SNV_CNV_combined_Feb7_tadaname_null_PPA_p0_0.94.txt
  ...
  fr<-function(x){ # x is the RR of other regulatory features, and splicing. 
    all_rr = x[1:length(non_splicing_features)] # all_rr is the all regulatory features not including splicing
    cal_logP_Zg1 <- function(data_partition_element){
      cal_logP_Zg1_level2 <-function(data_partition_element_level2){
        data_partition_element_level2[[2]]+data_partition_element_level2[[4]]*data_partition_element_level2[[1]][non_splicing_features]%*%all_rr-data_partition_element_level2[[3]]*exp(data_partition_element_level2[[1]][non_splicing_features]%*%all_rr)-data_partition_element_level2[[5]]
      }
      sum(sapply(data_partition_element, cal_logP_Zg1_level2))
    }
    ...
    splcing_rr = x[length(non_splicing_features)+1]
    splicing_cal_logP_Zg1 <- function(data_partition_element){
      splicing_cal_logP_Zg1_level2 <-function(data_partition_element_level2){
        data_partition_element_level2[[2]]+data_partition_element_level2[[4]]*data_partition_element_level2[[1]]%*%splcing_rr-data_partition_element_level2[[3]]*exp(data_partition_element_level2[[1]]%*%splcing_rr)-data_partition_element_level2[[5]]
      }
      sum(sapply(data_partition_element, splicing_cal_logP_Zg1_level2))
    }
    
    splicing_cal_logP_Zg0 <- function(data_partition_element){
      splicing_cal_logP_Zg0_level2 <-function(data_partition_element_level2){
        data_partition_element_level2[[2]]-data_partition_element_level2[[3]]-data_partition_element_level2[[5]]
      }
      sum(sapply(data_partition_element, splicing_cal_logP_Zg0_level2))
    }
    
    splicing_logP_Zg1 = sapply(splicing_data, splicing_cal_logP_Zg1)
    splicing_logP_Zg0 = sapply(splicing_data, splicing_cal_logP_Zg0)
    
    splicing_logP_table<-data.table(logP_Zg1 = splicing_logP_Zg1, logP_Zg0 = splicing_logP_Zg0, genename = names(splicing_logP_Zg1))
    
    splicing_logP_table <- splicing_logP_table[gene_prior, on = "genename"]
    splicing_logP_table <- splicing_logP_table[complete.cases(splicing_logP_table)]
    # get sum of ll of splicing mutations
    ll_sum2 <- sum(by(splicing_logP_table, splicing_logP_table$genename, function(x){log(exp(log(x[1,]$prior)+sum(x$logP_Zg1))+exp(log(1-x[1,]$prior)+sum(x$logP_Zg0)))}))
    
    return(ll_sum1 + ll_sum2)
  }
  # Use [optim] to do optimization
  optimization_time <- system.time(mle <- optim(rep(0.1, length(non_splicing_features) +1), fr,control=list("fnscale"=-1), hessian = TRUE))
...
# will read in [data_file] by chunk. 
# will take output from [adjust_mutation_rate_window_to_base_compact_more_feature_splicing_hybrid], which contains splicing info in a partitioned compact form
# This v2 version optimize splicing RR and other RRs in two separate mles. 
estimate_effect_size_for_simulation_data_mixture_with_categorization_for_compact_data_splicing_hybrid_v2<-function(data, splicing_data, non_splicing_features , gene_prior_file, sample_size = 314){
  #[data] is the object [base_info] returned from [adjust_mutation_rate_window_to_base_compact_more_feature_splicing_hybrid], which has compact base-level information. 
  #[splicing_data] is the object [splicing_info] returned from [adjust_mutation_rate_window_to_base_compact_more_feature_splicing_hybrid]
  # [non_splicing_features] is a vector indicating which non-splicing features need to be estimated for RR. e.g., c(2,3), meaning the 2nd and 3rd feature.
  #[gene_prior_file], a file that has prior (derived from posterior and prior)for a gene as a risk gene. genename \t probability e.g. ../other_annotation/gene_list/TADA_SNV_CNV_combined_Feb7_tadaname_null_PPA_p0_0.94.txt
  ...
  gene_prior$genename = as.character(gene_prior$genename)
  # notice the fr function is different from the function that deals with dataset without partition, needs to consider splicing mutation together
  fr_1<-function(x){ # x is the RR of other regulatory features, and splicing. 
  ...
  feature_number <- length(non_splicing_features)
  if (feature_number == 1){
    optimization_time_1 <- system.time(mle_1 <- optim(rep(0.1, feature_number), fr_1, method = "Brent", lower = -1, upper = 5,control=list("fnscale"=-1), hessian = TRUE))
  }
  else{
    optimization_time_1 <- system.time(mle_1 <- optim(rep(0.1, feature_number), fr_1 ,control=list("fnscale"=-1), hessian = TRUE))
  }
  
  # Use [optim] to do optimization for splicing mutations
  optimization_time_2 <- system.time(mle_2 <- optim(c(0.1), fr_2, method = "Brent", lower = -1, upper = 5,control=list("fnscale"=-1), hessian = TRUE))
  
  list(optimization_time_1 = optimization_time_1, optimization_time_2 = optimization_time_2, mle_1 = mle_1, mle_2 = mle_2)
}
...
# will take output from [adjust_mutation_rate_window_to_base_compact_more_feature_splicing_hybrid], which contains splicing info in a partitioned compact form
# This v2 version optimize splicing RR and other RRs in two separate mles. 
# This version will take into account compact data from multiple studies, and estimate relative risk. 
estimate_effect_size_for_simulation_data_mixture_with_categorization_for_compact_data_splicing_hybrid_multiple_studies<-function(data_list, splicing_data_list, non_splicing_features , gene_prior_file){
  #[data] is the a list of object [base_info] returned from [adjust_mutation_rate_window_to_base_compact_more_feature_splicing_hybrid], which has compact base-level information. 
  #[splicing_data] is a list of the object [splicing_info] returned from [adjust_mutation_rate_window_to_base_compact_more_feature_splicing_hybrid]
  # [non_splicing_features] is a vector indicating which non-splicing features need to be estimated for RR. e.g., c(2,3), meaning the 2nd and 3rd feature.
  #[gene_prior_file], a file that has prior (derived from posterior and prior)for a gene as a risk gene. genename \t probability e.g. ../other_annotation/gene_list/TADA_SNV_CNV_combined_Feb7_tadaname_null_PPA_p0_0.94.txt
  ...
  fr_2 <- function(x){
    ...
    ll_sum2 <- 0
    for(splicing_data in splicing_data_list){
      splicing_logP_Zg1 = sapply(splicing_data, splicing_cal_logP_Zg1)
      splicing_logP_Zg0 = sapply(splicing_data, splicing_cal_logP_Zg0)
      ...
      splicing_logP_table <- splicing_logP_table[complete.cases(splicing_logP_table)]
      # get sum of ll of splicing mutations
      ll_sum2 <- ll_sum2 + sum(by(splicing_logP_table, splicing_logP_table$genename, function(x){log(exp(log(x[1,]$prior)+sum(x$logP_Zg1))+exp(log(1-x[1,]$prior)+sum(x$logP_Zg0)))}))
    }
    ll_sum2
  }
  # Use [optim] to do optimization, for non-splicing_mutations
...
bayes_factor_for_each_gene_base_level_from_collapsed_data <- function(mut_collapsed_data,features, rr, coding_bayes_table_file, TADA_p0 = 0.94){
  # [mut_collapsed_data] is (output from [adjust_mutation_rate_window_to_base_compact_more_feature_v2]$info_base).
...
# all the features that need to be used are already encoded in a collapsed form the in the input data (output from [adjust_mutation_rate_window_to_base_compact_more_feature_v2]$info_base).
# so if splicing info hasn't been included in the data in collapsed form, splicing mutations won't be carried in the analysis.
# This version takes a list of [mut_collpased_data], it is used to generate BF for each gene from mutation count of each study.
bayes_factor_for_each_gene_base_level_from_collapsed_data_multiple_studies <- function(mut_collapsed_data_list,features, rr, coding_bayes_table_file, TADA_p0 = 0.94){
  ...
  for(mut_collapsed_data in mut_collapsed_data_list){
    logP_Zg1 <- sapply(mut_collapsed_data, cal_logP_Zg1)
    logP_Zg0 <- sapply(mut_collapsed_data, cal_logP_Zg0)
...
generate_BF_for_splicing_mutations <- function(mut_file, 
  ...
  output_splicing_logBF$BF_splicing <- exp(output_splicing_logBF$logBF_splicing)
  list(splicing_BF_table = output_splicing_logBF)
}


# This function calculates splicing BF for genes after combining information from multiple studies.
generate_BF_for_splicing_mutations_multiple_studies <- function(splicing_info_list,
                                                                log_splicing_rr = log(2.1)){
  # [splicing_info_list] is a list of [splicing_info] returned from function [adjust_mutation_rate_window_to_base_compact_more_feature_splicing_hybrid]
  # [log_splicing_rr] the relative risk of splicing mutations on the log scale.
  
  # define a function to calculate splicing logBF for each gene
  splicing_logBF_per_gene <- function(splicing_info_for_single_gene){
    logBF <- splicing_info_for_single_gene$sum_mut_count * 1 * log_splicing_rr - splicing_info_for_single_gene$sum_mut_rate * (exp(1 * log_splicing_rr) - 1)
    logBF
  }
  
  splicing_logBF_per_study <- function(splicing_info){
    splicing_logBF_table_per_study <- data.table(genename = rep("fake",length(splicing_info)), logBF_splicing = rep(0,length(splicing_info)))
    for(i in 1:length(splicing_info)){
      splicing_logBF_table_per_study[i,]$logBF_splicing <- splicing_logBF_per_gene(splicing_info[[i]][[1]])
      splicing_logBF_table_per_study[i,]$genename <- names(splicing_info[i])
    }
    splicing_logBF_table_per_study
  }
  
  # a list of splicing logBF from each study
  splicing_logBF_list <- sapply(splicing_info_list, splicing_logBF_per_study,simplify = FALSE)
  
  # now combine study together by adding the logBF for each gene up
  
  output_splicing_logBF <- data.table(genename = splicing_logBF_list[[1]]$genename, logBF_splicing = 0)
  for(i in 1:length(splicing_logBF_list)){
    output_splicing_logBF$logBF_splicing <- output_splicing_logBF$logBF_splicing + splicing_logBF_list[[i]]$logBF_splicing
  }
